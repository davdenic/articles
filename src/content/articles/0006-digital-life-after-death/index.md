...
- **GitHub — a designated *successor*.** As a developer this is the one I'd forgotten about: you can name a successor in advance to manage your repositories if you die; failing that, next of kin have to file a support request with ID and a death certificate. And that's the *good* case.
- **Microsoft, AWS, Azure — basically nothing to set up in advance.** This is the part that unsettled me most, given how much of my working life lives here. A Microsoft account has no legacy contact at all; from what I can tell its content is only released under a court order, and a dormant account simply auto-deletes after two years (Germany and China look like exceptions). AWS and Azure don't seem to have a successor feature either — I couldn't find an official deceased-user policy for them at all, just "open a support case" and let the estate paperwork grind. The cloud quietly assumes an *admin* will take over, not that someone might die holding the only keys.
...
Oddly, I've made my peace with the delete-after-dormancy default. Most accounts probably *should* quietly fade once no one has touched them for years — the digital tidying-up none of us will ever get around to. The catch is the handful of things you actually want to keep. My best friend passed away, and his Facebook page is still there, memorialized. Every so often, when the mood takes me, I still write on it. It's not rational and it doesn't reach him — but it's a place that still holds a bit of him, and a blanket "delete after two years" would quietly erase that. So maybe the honest default isn't *delete* or *keep*, but letting people mark the few things worth outlasting them.
