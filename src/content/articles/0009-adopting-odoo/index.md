...
| Performance tuning | One server to profile and tune | Several services to tune, each its own way |
...
- **A side note: real foreign keys** — TYPO3 Core doesn't enforce database foreign keys; relations live in its TCA config ([Forge #84639](https://forge.typo3.org/issues/84639)). Odoo's ORM maps them to real PostgreSQL FKs ([ORM reference](https://www.odoo.com/documentation/17.0/developer/reference/backend/orm.html)). So referential integrity went from application-enforced to database-enforced — a small thing, but reassuring for a web of interlinked entities.
