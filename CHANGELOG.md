...
- Home mosaic now renders SVG hero images: the card-image glob and resolver excluded `.svg`, so articles with an SVG `image` (e.g. `0006`) fell back to a blank tile. SVG heroes are now used as-is (no `getImage`/sharp transform, which can't emit webp from SVG).
